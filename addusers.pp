user { "luftloch":
  ensure => present,
  groups => ['sudo'],
  shell => '/bin/bash',
  managehome => true,
  require => [ Package['sudo'] ];
}

file { '/home/luftloch/.ssh/':
  ensure => directory,
  owner => 'luftloch',
  group => 'luftloch',
  mode => '0700',
  require => [ User['luftloch'] ];
}

ssh_authorized_key { "luftloch":
  user => 'luftloch',
  type => 'ssh-rsa',
  key  => '',
  require => [ File['/home/luftloch/.ssh/'] ];
}

exec { "luftloch_password":
  command => "/bin/bash -c \"/usr/sbin/chpasswd <<< 'luftloch:8OT3dnNnLOmy+nwMhz+X+jua0vFtspqfDaCarsn2Lc8='\"",
}

file { '/home/luftloch/password':
  ensure => file,
  owner => 'luftloch',
  group => 'luftloch',
  mode => '0700',
  content => "Change this password asap!\npassword: 8OT3dnNnLOmy+nwMhz+X+jua0vFtspqfDaCarsn2Lc8=\n",
  require => [ User['luftloch'] ];
}

user { "tarnatos":
  ensure => present,
  groups => ['sudo'],
  shell => '/bin/bash',
  managehome => true,
  require => [ Package['sudo'] ];
}

file { '/home/tarnatos/.ssh/':
  ensure => directory,
  owner => 'tarnatos',
  group => 'tarnatos',
  mode => '0700',
  require => [ User['tarnatos'] ];
}

ssh_authorized_key { "cbreut@cbook-UB":
  user => 'tarnatos',
  type => 'ssh-rsa',
  key  => '',
  require => [ File['/home/tarnatos/.ssh/'] ];
}

exec { "tarnatos_password":
  command => "/bin/bash -c \"/usr/sbin/chpasswd <<< 'tarnatos:JTU6qM0CGRWAjE+hpKXajiTKHbuiROZK+i6vk+Sy49c='\"",
}

file { '/home/tarnatos/password':
  ensure => file,
  owner => 'tarnatos',
  group => 'tarnatos',
  mode => '0700',
  content => "Change this password asap!\npassword: JTU6qM0CGRWAjE+hpKXajiTKHbuiROZK+i6vk+Sy49c=\n",
  require => [ User['tarnatos'] ];
}

user { "fsteinel":
  ensure => present,
  groups => ['sudo'],
  shell => '/bin/bash',
  managehome => true,
  require => [ Package['sudo'] ];
}

file { '/home/fsteinel/.ssh/':
  ensure => directory,
  owner => 'fsteinel',
  group => 'fsteinel',
  mode => '0700',
  require => [ User['fsteinel'] ];
}

ssh_authorized_key { "grill@calculon":
  user => 'fsteinel',
  type => 'ssh-rsa',
  key  => '',
  require => [ File['/home/fsteinel/.ssh/'] ];
}

exec { "fsteinel_password":
  command => "/bin/bash -c \"/usr/sbin/chpasswd <<< 'fsteinel:G+8kkV5Fou5olNagE+U8VhMUYsADlrixb2ELgy0kgls='\"",
}

file { '/home/fsteinel/password':
  ensure => file,
  owner => 'fsteinel',
  group => 'fsteinel',
  mode => '0700',
  content => "Change this password asap!\npassword: G+8kkV5Fou5olNagE+U8VhMUYsADlrixb2ELgy0kgls=\n",
  require => [ User['fsteinel'] ];
}

user { "rubo77":
  ensure => present,
  groups => ['sudo'],
  shell => '/bin/bash',
  managehome => true,
  require => [ Package['sudo'] ];
}

file { '/home/rubo77/.ssh/':
  ensure => directory,
  owner => 'rubo77',
  group => 'rubo77',
  mode => '0700',
  require => [ User['rubo77'] ];
}

ssh_authorized_key { "ruben@nebelhaufen":
  user => 'rubo77',
  type => 'ssh-rsa',
  key  => '',
  require => [ File['/home/rubo77/.ssh/'] ];
}

exec { "rubo77_password":
  command => "/bin/bash -c \"/usr/sbin/chpasswd <<< 'rubo77:9WneYGhBnVkptzzMwS6oXQ/Jny4E9jJf0IrHTAM28PU='\"",
}

file { '/home/rubo77/password':
  ensure => file,
  owner => 'rubo77',
  group => 'rubo77',
  mode => '0700',
  content => "Change this password asap!\npassword: 9WneYGhBnVkptzzMwS6oXQ/Jny4E9jJf0IrHTAM28PU=\n",
  require => [ User['rubo77'] ];
}

user { "mikevanbike":
  ensure => present,
  groups => ['sudo'],
  shell => '/bin/bash',
  managehome => true,
  require => [ Package['sudo'] ];
}

file { '/home/mikevanbike/.ssh/':
  ensure => directory,
  owner => 'mikevanbike',
  group => 'mikevanbike',
  mode => '0700',
  require => [ User['mikevanbike'] ];
}

ssh_authorized_key { "luci@lea":
  user => 'mikevanbike',
  type => 'ssh-rsa',
  key  => '',
  require => [ File['/home/mikevanbike/.ssh/'] ];
}

exec { "mikevanbike_password":
  command => "/bin/bash -c \"/usr/sbin/chpasswd <<< 'mikevanbike:zvyW173U+3knF/h4fVwE/WptiKUlYNzYbfPZ2YCC89s='\"",
}

file { '/home/mikevanbike/password':
  ensure => file,
  owner => 'mikevanbike',
  group => 'mikevanbike',
  mode => '0700',
  content => "Change this password asap!\npassword: zvyW173U+3knF/h4fVwE/WptiKUlYNzYbfPZ2YCC89s=\n",
  require => [ User['mikevanbike'] ];
}

user { "sargon":
  ensure => present,
  groups => ['sudo'],
  shell => '/bin/bash',
  managehome => true,
  require => [ Package['sudo'] ];
}

file { '/home/sargon/.ssh/':
  ensure => directory,
  owner => 'sargon',
  group => 'sargon',
  mode => '0700',
  require => [ User['sargon'] ];
}

ssh_authorized_key { "sargon@pan":
  user => 'sargon',
  type => 'ssh-rsa',
  key  => '',
  require => [ File['/home/sargon/.ssh/'] ];
}

exec { "sargon_password":
  command => "/bin/bash -c \"/usr/sbin/chpasswd <<< 'sargon:zo3xJ2uLThuVk6pqT8fSDNiRLfgiq2be7KCH5cTsdvg='\"",
}

file { '/home/sargon/password':
  ensure => file,
  owner => 'sargon',
  group => 'sargon',
  mode => '0700',
  content => "Change this password asap!\npassword: zo3xJ2uLThuVk6pqT8fSDNiRLfgiq2be7KCH5cTsdvg=\n",
  require => [ User['sargon'] ];
}

user { "wonka":
  ensure => present,
  groups => ['sudo'],
  shell => '/bin/bash',
  managehome => true,
  require => [ Package['sudo'] ];
}

file { '/home/wonka/.ssh/':
  ensure => directory,
  owner => 'wonka',
  group => 'wonka',
  mode => '0700',
  require => [ User['wonka'] ];
}

ssh_authorized_key { "w@chaos":
  user => 'wonka',
  type => 'ssh-rsa',
  key  => '',
  require => [ File['/home/wonka/.ssh/'] ];
}

exec { "wonka_password":
  command => "/bin/bash -c \"/usr/sbin/chpasswd <<< 'wonka:gBmPUJKZuiqcI61gqyEkZlokH8fXOQVR6dco/upW1tE='\"",
}

file { '/home/wonka/password':
  ensure => file,
  owner => 'wonka',
  group => 'wonka',
  mode => '0700',
  content => "Change this password asap!\npassword: gBmPUJKZuiqcI61gqyEkZlokH8fXOQVR6dco/upW1tE=\n",
  require => [ User['wonka'] ];
}

package { 'sudo': ensure=>installed; }
